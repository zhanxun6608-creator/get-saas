...
export const metadata: Metadata = {
  ...
  description: 'Browse AI agents for browser automation, research, coding, and task execution.',
}
      ...
      <nav className="border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <Link href="/agents" className="text-sm text-indigo-400">Agents</Link>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tools.map(tool => {
            ...
            return (
              <Link key={tool.id} href={`/agents/${tool.slug}`} className="group bg-[#111827] border border-white/5 rounded-2xl p-6 flex flex-col hover:border-indigo-500/30 hover:shadow-lg hover:shadow-indigo-500/5 transition-all">
                <div className="flex items-center justify-between mb-3">
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${cat.color}`}>{cat.label}</span>
                  {tool.featuredRank && tool.featuredRank <= 3 && <span className="text-xs text-amber-400 font-medium">Featured</span>}
                </div>
