    ...
    <div className="min-h-screen bg-[#0b0f19] text-slate-300">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-16">
        ...
        <section className="text-center space-y-4">
          <h1 className="text-[36px] md:text-[44px] font-bold text-white leading-tight">{task.title}</h1>
          <p className="text-lg text-slate-400 max-w-2xl mx-auto">{task.description}</p>
          {recommendedTools[0] && (
            <a
                ...
                {recommendedTools.map(tool => (
                  <tr key={tool.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        tool.automationLevel === 'high' || tool.automationLevel === 'autonomous'
                          ? 'bg-green-500/10 text-green-400' : tool.automationLevel === 'medium'
                          ? 'bg-amber-500/10 text-amber-400' : 'bg-slate-500/10 text-slate-400'
                    ...
                    <td className="py-3 pl-4">
                      <a href={`/go/${tool.slug}`} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300 text-xs font-medium whitespace-nowrap">Try →</a>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="space-y-6">
              {task.workflowSteps.map((step, i) => (
                <div key={i} className="flex gap-4">
                  <div className="flex-shrink-0 w-10 h-10 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 font-bold text-sm">{i + 1}</div>
        ...
        {premiumWorkflow && (
          <section className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-2xl p-8 text-center">
            ...
            <Link href={`/workflows/${premiumWorkflow.slug}`} className="inline-block px-8 py-3 bg-white text-[#0b0f19] font-semibold rounded-xl hover:bg-slate-200 transition-all">
        ...
        <section className="bg-[#111827] border border-white/5 rounded-2xl p-8 text-center">
          ...
          <form action="https://app.convertkit.com/forms/placeholder" method="post" className="flex gap-3 max-w-md mx-auto">
            <input type="email" name="email_address" placeholder="Your email address" className="flex-1 px-4 py-3 bg-[#0b0f19] border border-white/10 rounded-xl text-white placeholder:text-slate-600 focus:outline-none focus:border-indigo-500/50" required />
            <button type="submit" className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all whitespace-nowrap">Subscribe</button>
          </form>
        </section>
      </main>
