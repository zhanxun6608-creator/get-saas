...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${tool.name} — AI Agent Review | DoWithAI`,
...
export default async function AgentPage({ params }: Props) {
  ...
  const relatedTasks = allTasks.filter(t =>
    t.recommendedToolIds?.includes(tool.id)
  )
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-12">
        ...
        <section>
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className="px-3 py-1 rounded-full text-xs font-medium border bg-indigo-500/10 text-indigo-400 border-indigo-500/20">
          ...
          <div className="mt-6 flex gap-4">
            ...
            <a
              ...
              className="px-8 py-3 border border-white/10 text-slate-300 font-medium rounded-xl hover:border-indigo-500/30 transition-all"
            >
              Visit Website
            </a>
          </div>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-[#111827] border border-white/5 rounded-xl p-4 text-center">
            ...
            <div className="text-sm font-medium text-white">{tool.pricing}</div>
          </div>
          <div className="bg-[#111827] border border-white/5 rounded-xl p-4 text-center">
            <div className="text-xs text-slate-500 mb-1">Automation</div>
            <div className="text-sm font-medium text-white capitalize">{tool.automationLevel}</div>
          </div>
          <div className="bg-[#111827] border border-white/5 rounded-xl p-4 text-center">
            <div className="text-xs text-slate-500 mb-1">Ease of Use</div>
            <div className="text-sm font-medium text-white capitalize">{tool.easeOfUse}</div>
          </div>
          <div className="bg-[#111827] border border-white/5 rounded-xl p-4 text-center">
            <div className="text-xs text-slate-500 mb-1">Category</div>
            <div className="text-sm font-medium text-white capitalize">{tool.category?.replace('_', ' ')}</div>
          </div>
        </section>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {relatedTasks.map(task => (
                <Link
                  ...
                  className="bg-[#111827] border border-white/5 rounded-xl p-5 hover:border-indigo-500/30 transition-all group"
                >
