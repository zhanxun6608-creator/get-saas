...
export default async function WorkflowPage({ params }: Props) {
  ...
  const buyUrl = wf.stripePaymentLink && wf.stripePaymentLink !== '#'
    ? wf.stripePaymentLink
    : null
                ...
                <div className="space-y-4">
                  {wf.steps.map((step, i) => (
                    <div key={i} className="flex gap-4">
                      <div className="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 font-bold text-xs">
                        {i + 1}
                      </div>
                      <div className="bg-[#111827] border border-white/5 rounded-xl p-4 flex-1">
              ...
              <section>
                ...
                <div className="flex flex-wrap gap-2">
                  {wf.toolsIncluded.map((t, i) => (
                    <span key={i} className="px-3 py-1.5 bg-indigo-500/10 text-indigo-400 text-sm rounded-full border border-indigo-500/20">{t}</span>
                  ))}
                </div>
              </section>
            )}
            ...
            <div className="sticky top-8 bg-[#111827] border border-white/5 rounded-2xl p-6 space-y-4">
              ...
              {buyUrl ? (
                <a
                  ...
                  className="block text-center w-full py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all"
                >
                  Buy Workflow Pack
                </a>
              ) : (
                <div className="block text-center w-full py-3 bg-slate-700 text-slate-400 font-semibold rounded-xl cursor-not-allowed">
                  Coming Soon
                </div>
              )}
