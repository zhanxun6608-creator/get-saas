...
export const metadata: Metadata = {
  ...
  description: 'Premium AI automation workflow packs with step-by-step SOPs, prompt templates, and tool stacks.',
}
    ...
    <div className="min-h-screen bg-[#0b0f19] text-slate-300">
      <nav className="border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <Link href="/workflows" className="text-sm text-indigo-400">Workflows</Link>
          </div>
        </div>
      </nav>
      <main className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {workflows.map(wf => (
            <Link key={wf.id} href={`/workflows/${wf.slug}`} className="group bg-[#111827] border border-white/5 rounded-2xl p-6 flex flex-col hover:border-indigo-500/30 hover:shadow-lg hover:shadow-indigo-500/5 transition-all">
              ...
              {wf.toolsIncluded && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {wf.toolsIncluded.map((t, i) => <span key={i} className="px-2 py-0.5 bg-indigo-500/10 text-indigo-400 text-xs rounded-full">{t}</span>)}
                </div>
              )}
